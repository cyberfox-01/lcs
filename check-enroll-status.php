...
<html>
  ...
  <body>
    ...
    <section id="showcase">
      <div class="container">
        <form class="container" method ="post" action = "check-enroll-status.php">
           <div class = "tuition-group">
              <div>
                <h2> Enrollment Satus</h2>
                <p> Welcome to the enrollment status page. Enter you Temporary Reference Number or Student Number to view status. </p>
                 ...
                 <tr bgcolor ="#E1F0F0" style="text-align:left">
                  ...
                  <?php
        			...
        			$con = new mysqli('localhost', 'root', 'Dont4GET', 'lcs');   
                      $sql = $con-> query("SELECT enrollmentStatus, studentID, gradeLevel, enrolledWhen, enrolleeFirstName, enrolleeLastName, enrolleeCode  FROM enrollee WHERE LOWER(enrolledByEmailAddress) = '$enrolledByEmailAddress' AND enrollmentStatus IN ('New', 'Pending', 'For Interview', 'For Exam', 'For Payment', 'Enrolled')");
                      while($row = mysqli_fetch_array($sql))
                        {
                        
                        echo "<tr>";
                        echo "<td><a href='http://tercerotaton.cf/lcs/view-new-student-info.php?studentID=$row[studentID]'>". $row['studentID'] . "</a></td>";
